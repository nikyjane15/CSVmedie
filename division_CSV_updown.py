...
for directory in dirs:
    ...
    for pkt_scale in new_path_dirs:
        ...
        if a[0] == 0:
            start = df_limits.loc[0, 'Service stop responding time (ns)']
            end = df_limits.loc[1, 'Service stop responding time (ns)']
            limit_timestamp.append((df_limits.loc[0, 'Scale type'], start, end))
            ...
            start = df_limits_last['Receiving time (ns)'].iloc[0]
            end = df_limits.loc[0, 'Service stop responding time (ns)']
            limit_timestamp.append(("UP", start, end))
        ...
        for dir in dir_results:
            ...
            save_dir = Path.joinpath(save_path,dir)
            ...
            for file in file_list:
                ...
                if not mean:
                    df_new.to_csv(Path(save_dir, f"scale_" + file), index=False, sep=';', encoding='utf-8')
